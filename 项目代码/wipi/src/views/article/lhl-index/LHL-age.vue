    ...
    <div class="all">
        ...
        <ul class="lhl-ull">
            <li v-for="item in Lhlobj" :key="item.id">
                <div class="lhl-head">
                    <h3 class="ate" @click="handleDetail(item.id)">
                        {{ item.title }}
                    </h3>
                    <span>8天前</span>
...
export default {
    ...
    computed: {
        ...
        newList() {
            return this.articleList.filter((item) => item.category === "123");
        },
    },
    mounted() {
        let joi = this.articleList.filter((item) => {
            return item.category && item.category.label === "123";
        });
        console.log(joi);
        this.Lhlobj = joi;
    },
    methods: {
        ...
        handleDetail(id) {
            window._hmt.push(["_trackEvent", '文章', "lookDetail", "详情id", id]);
